...
Pour l'exponentiation rapide, on sait qu'on effectue dans le pire des cas $\Theta(\text{log t})$ produits de matrices, soit une complexité en $\Theta(T_{prod}(N^2,p)\times \text{log t}) = \Theta \left( max \left( pL + N^4 b, \frac{N^6}{p} \right) \times \text{log t} \right) $ pour le calcul de $\Delta^t$. \\ % TODO : Retrouver la complexité pire cas pouwr exponentiation rapide, et remplacer T_prod
