...
% TODO : Other package for algorithms ?
...
% TODO : L'initialisation ne pose pas de problèmes, mais le broadcast de la matrice ( ainsi que des autres données ) peut prendre un certain temps, étant donné que le réseau est un réseau torique

Pour l'exponentiation rapide, on sait qu'on effectue dans le pire des cas $O(\text{log t})$ produits de matrices, soit une complexité en $O(T_{prod}(N^2,p)\times \text{log t})$ pour le calcul de $\Delta^t$. \\ % TODO : Retrouver la complexité pire cas pour exponentiation rapide, et remplacer T_prod

Enfin, le dernier produit matrice/vecteur se fait en temps $T_{prodVect} = $. \\ % TODO : retrouver la complexité

D'où une complexité temporelle finale en ... % TODO : terminer le calcul

\subsubsection*{Complexité en espace}

Concernant la complexité temporelle, chaque processeur dispose d'un bloc de $\Delta$ de taille $\left(\frac{N^2}{p}\right)^2$, d'un bloc de $X^0$ de taille $\frac{N^2}{p}$, mais également d'un autre bloc de taille $\left(\frac{N^2}{p}\right)^2$ nécessaire au calcul de $\Delta^t$. Il ne faut pas non plus oublier que des blocs temporaires doivent être alloués pour les envois et réceptions de lignes et de colonnes durant les étapes où on effectue des multiplications. % TODO : Qu'est ce que celà représente comme mémoire ?

\subsubsection*{Comparaison dans le cas général} 

% TODO : Qu'est ce que ca veut dire ?? Comparer par rapport à la question 3 ?
...
% TODO : Adapter algorithme produit de matrice quand le réseau forme une clique
