...
typedef struct list sourcelist;
...
void init_datas()
{
	...
	neighbor_matrix = calloc((nb_col-2)*(nb_row-2), sizeof(double));
...
//set 0 to workmatrix content
void zero_work()
{
	int i;
	for(i=0;i<(nb_col-2)*(nb_row-2);i++)
		work_matrix[i]=0;
	for(i=0;i<nb_col;i++)
	{
		work_first_row[i]=0;
...
void compute_image(double p) 
{
	...
	MPI_Recv(neighbor_last_row, nb_col, MPI_DOUBLE, north, 2, MPI_VERTICAL, MPI_STATUS_IGNORE);
	MPI_Recv(neighbor_first_row, nb_col, MPI_DOUBLE, south, 1, MPI_VERTICAL, MPI_STATUS_IGNORE);
	MPI_Recv(neighbor_last_col, nb_row, MPI_DOUBLE, west, 4, MPI_HORIZONTAL, MPI_STATUS_IGNORE);
	MPI_Recv(neighbor_first_col, nb_row, MPI_DOUBLE, east, 3, MPI_HORIZONTAL, MPI_STATUS_IGNORE);
...
void addheat(int i,int j,double t)
{
	i-=my_col*nb_col;
	j-=my_row*nb_row;
...
double getlocalheat(int i,int j,double* m,double* fr,double* lr,double* fc,double* lc)
{
...
{
	int N=(int)sqrt(nb_proc);
	MPI_Bcast(neighbor_first_row, nb_col, MPI_DOUBLE, proc, MPI_COMM_WORLD);
	MPI_Bcast(neighbor_last_row, nb_col, MPI_DOUBLE, proc, MPI_COMM_WORLD);
	MPI_Bcast(neighbor_first_col, nb_row, MPI_DOUBLE, proc, MPI_COMM_WORLD);
	MPI_Bcast(neighbor_last_col, nb_row, MPI_DOUBLE, proc, MPI_COMM_WORLD);
	MPI_Bcast(neighbor_matrix, (nb_row-2)*(nb_col-2), MPI_DOUBLE, proc, MPI_COMM_WORLD);
	sourcelist* s=sources;
	while(s!=NULL)
	{
		rotatesetheatapply(proc%N,proc/N,s->i,s->j,s->t,neighbor_matrix,neighbor_first_row,neighbor_last_row,neighbor_first_col,neighbor_last_col);
		s=s->next;
	}
